/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package CumulativeActivities;

import java.util.Scanner;

/**
 *
 * @author ta
 */
public class InterestCalculator {
    public static void main(String[] args) {
        Scanner myScanner= new Scanner((System.in));
        System.out.println("How much do you want to invest?");
        float principle = myScanner.nextFloat();
        System.out.println("How many years are investing?");
        int years = myScanner.nextInt();
        System.out.println("What is the annual interest rate % growth?");
        float annualInterest = myScanner.nextFloat();
        
        System.out.println("What is the type of interest compound periods:(1) daily,"
                + " (2) monthly (3) quarterly (4) bi-annually or (5) annually?");
        int interestType = myScanner.nextInt();
        
        float earned=0;
        for (int i = 1; i < years+1; i++) {
            System.out.println("Year " + i + ":");
            System.out.println("Begin with $" + principle);
            switch(interestType){
                case 1:
                    earned = earnInterest(principle, annualInterest, 365);
                    break;
                case 2:
                    earned = earnInterest(principle, annualInterest, 12);
                    break;
                case 3:
                    earned = earnInterest(principle, annualInterest, 4);
                    break;
                case 4:
                    earned = earnInterest(principle, annualInterest, 2);
                    break;
                case 5:
                    earned = earnInterest(principle, annualInterest, 1);
                    break;
                
            }
            System.out.println("Earned: " + earned);
            System.out.println("Ended with: " + (principle + earned));
             principle +=earned;
        }
    }
    public static float earnInterest(float princip, float interest, int period){
        float earned=0;
        for (int j = 0; j < period; j++) {
            earned += princip *(interest/(period*100));
    //        System.out.println("earned: " +earned);
            princip +=earned;
        }
        return earned;
    }
}
